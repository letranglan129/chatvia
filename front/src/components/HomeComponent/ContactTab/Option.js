...
import { toggleDropdown } from '../../../redux/popperSlice'
...
function Option({ index, friend }) {
    ...
    const openChat = async () => {
        ...
        dispatch(changeCurrentChat({ ...friend.conversation, name: friend?.name }))
            ...
            {openDropdownListContact === index && (
                ...
                >
                    <ul className="border border-gray-500 rounded-lg py-2 w-40 bg-gray-100 dark:bg-gray-800 overflow-hidden">
                        <li onClick={openChat} className="flex items-center justify-between py-1 px-3 cursor-pointer hover:bg-gray-300 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:bg-opacity-5">
                            <p>Trò chuyện</p>
