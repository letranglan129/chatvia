...
import { DEFAULT_IMG, SWIPER_THEME } from '../../../image'
import { zoomIn, zoomOut, reset, rotate, wheelZoom } from '../../../redux/swiperSlice'
	...
	return (
		<div className='img-box relative w-full h-full'>
			<div className='absolute top-4 left-4 z-10 px-2 bg-gray-800 rounded-full flex items-center bg-opacity-50'>
				<button onClick={zoomInDispatch} className='text-center px-2 w-10 h-10' title='Phóng to'>
					<img className='control-image' src={SWIPER_THEME.zoomIn} alt='Zoom in' />
				</button>
				<button onClick={zoomOutDispatch} className='text-center px-2 w-10 h-10' title='Thu nhỏ'>
					<img className='control-image' src={SWIPER_THEME.zoomOut} alt='Zoom out' />
				</button>
				<button onClick={resetDispatch} className='text-center px-2 w-10 h-10' title='Đưa về mặc định'>
					<img className='control-image' src={SWIPER_THEME.reset} alt='Reset' />
				</button>
				<button onClick={rotateDispatch} className='text-center px-2 w-10 h-10' title='Xoay'>
					<img className='control-image' src={SWIPER_THEME.rotate} alt='Rotate' />
				</button>
			</div>
			<div className='overflow-hidden border-red-500 flex items-center justify-center w-full h-full'>
				<Draggable nodeRef={nodeRef} disabled={!swiper.isDraggable}>
					<span ref={nodeRef} className='h-full flex items-center justify-center'>
						<img
