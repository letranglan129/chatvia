...
class ConversationController {
	...
	async getConversations(req, res) {
		try {
			...
			const conversationPromise = conversation?.map(async (item) => {
				const friendId = item?.members.find((id) => id !== req.user?._id.toString())
				const friend = await User.findById(friendId)
				const lastMessage = await Message.findOne({ conversationId: item._id }).sort({ createdAt: -1 }).limit(1)
	...
	async getOneConversation(req, res) {
		try {
			...
			const friendInfo = conversation?.members.find((id) => id !== user?._id.toString())
	...
	async getLastMessageByConversationId(req, res) {
		try {
			const { id } = req.params
			const lastMessage = await Message.findOne({ conversationId: id }).sort({ createdAt: -1 }).limit(1)
